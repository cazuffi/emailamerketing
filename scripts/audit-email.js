...
assert.strictEqual(greyCtaColumns.eq(0).attr('width'), '68%');
assert.strictEqual(greyCtaColumns.eq(1).attr('width'), '32%');
...
assert.match(greyCtaColumns.eq(0).attr('style') || '', /padding:0 16px 0 0/i);
...
featureCells.each((_, cell) => {
  assert.strictEqual($featureCardsFour(cell).attr('width'), '50%', 'Desktop card cells must be equal width');
...
featureGrid.find('.feature-card-body').each((_, body) => {
  assert.strictEqual($featureCardsFour(body).attr('height'), '220', 'Desktop card bodies must share an explicit height');
